...
On remarque que pour les données un peu atypique on a un facteur commun qui est le fait que ce soit en campagne CA2 pour la plupart qu'elles sont là. Il faudrait connaître plus d'information sur les conditions dans lesquelles ont été mené ces campagnes. 
...
On voit donc bien qu'en fonction de la campagne on obtient pas la même distribution dans nos capteurs chimiques et que la campagne CA2 est la campagne avec la plus grande variance et des valeurs globalement plus haute au niveau des capteurs chimiques. 
...
On a remarqué que dans les données certaines valeurs de type sont des "?" qui signifie un manque d'information sur le type. On compare pour les types des localisations où s'est produit l'anomalie. Pour la localisation P10 tous les types sont sourceindustrie. On a d'autant plus remarqué que pour toutes les localisations on associe qu'un seul type parmi (Rural, urbain, compostage...). On peut donc légitimement imputer le type sourceindustrie. 


Pour la localisation P22, on a pas d'autres localisation P22
...
```
Même si on voit que les valeurs captées sont à la localisation P22 sont proches des médianes des distributions dans chaque catégorie pour une même campagne. Le fait de ne pas avoir d'autre localisation et surtout pas de type ne sert pas à répondre aux questions évoquer en introduction à part pour la différence entre les campagnes d'hiver et d'été mais ppur le coup supprimer une ligne ne pose pas de problème.  
...
indices_quali <- (nb_col_num + 1):nb_col_total

print("Indices des variables qualitatives :")
print(indices_quali)
print("Noms de ces variables :")
print(colnames(data)[indices_quali])

res_correct <- PCA(data,
                   scale.unit = TRUE,   
...
```
Pour faire l'ACP on a normalisé les données. Il est important dans ce cas de faire la normalisation des données pour éviter la sur-domination des variables à grande variance, par exemple : Tot_OcNoDecana ayant pour 3Q - 1Q = 560 >> B avec 3Q - 1Q = 71. J'ai donc scale de manière linéaire toutes les variables. En plus on ne sait pas si les valeurs ont les mêmes unités. C'est probablement pas le cas. On a une décroissance de la variance expliquée par les composantes qui décroît de manière exponentielle. Il n'y a pas de cassures. On peut garder les composantes sui ont des valeurs propres supérieurs à 1 ou alors garder les composantes qui en somme dépasse les 90% de variances expliqués cumulé. 
...
```
Sur cette représentation de la répartition des données sur les 2 composantes principales. On voit que l'axe des absisses délimite source industrie 80 % en dessous à peu près. Faut-il retirer les point à l'extrémité droite en bas? 
```{r}
plot.PCA(res_correct, choix = "ind", 
       habillage = "SAISON",
       label = "quali", cex = 0.8,
...
```
On voit cette fois une délimitation claire entre été et hiver symbolisé par l'axe des abscisses. L'individu en bas à droite lui semble être mal classé. C'est un indiv d'été alors qu'il est sous la droite des abscisses. On va donc refaire l'ACP en éliminant cette individu. 

```{r}
data <- data[-idx_max_dim1, ]
nb_col_num <- ncol(data_num)
nb_col_total <- ncol(data)
indices_quali <- (nb_col_num + 1):nb_col_total

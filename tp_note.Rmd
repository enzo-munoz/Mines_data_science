...
```{r, echo = FALSE}
# D'abord, on crée un data.frame avec les statistiques
stats_df <- data.frame(
...
# Ensuite, on utilise kable() pour créer un joli tableau
# Le paramètre 'digits' permet d'arrondir joliment toutes les valeurs
knitr::kable(
  stats_df,
...
op <- par(mfrow = c(1,2))
ro <- acf(temperature, lag=15, ylim = c(-1,1), main = "ACF empirique")
alpha <- pacf(temperature, lag=15, ylim = c(-1,1), main = "et PACF", xlim=c(0,15))
par(op)
...
```{r, echo=FALSE}
# 1. EXÉCUTER LE TEST EN PREMIER (c'est l'étape la plus importante)
adf_test <- adf.test(des_data, alternative = "stationary")

# 2. PRÉPARER LA PARTIE CONDITIONNELLE DE L'INTERPRÉTATION
#    On stocke le résultat du if/else dans une variable.
...
# 3. ASSEMBLER LE TEXTE FINAL EN UTILISANT LA VARIABLE CRÉÉE
texte_final <- paste(
...
# D'abord le résultat brut du test
print(adf_test)

# Puis notre interprétation formatée

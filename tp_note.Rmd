...
acf(res_AR_1, lag.max = 36, xaxt = "n", main = "ACF des résidus AR(4)")
axis(1, at = 0:36/12, labels = 0:36)
acf(res_MA_1, lag.max = 36, xaxt = "n", main = "ACF des résidus ARMA(5,0,5)")
axis(1, at = 0:36/12, labels = 0:36)

Box.test(res_AR_1, lag=20, type="Ljung-Box")
Box.test(res_MA_1, lag=20, type="Ljung-Box")

hist(res_AR_1, main = "Histogramme des résidus AR(4)", xlab = "Résidus")
hist(res_MA_1, main = "Histogramme des résidus ARMA(5,0,5)", xlab = "Résidus")
```
Pas non plus de Ljung-Box
...
ArchTest(res_AR, lags=10)
ArchTest(residus, lags=10)
...
n_test <- 12
ts_train <- window(des_data, end = time(des_data)[length(temperature) - n_test])
ts_test <- window(des_data, start = time(des_data)[length(temperature) - n_test + 1])
